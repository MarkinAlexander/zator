...
beginner_guide_menu() {
  ...
  echo -e "${Fcyan}Обновления (Пункт 5 главного меню)${plain}"
  echo -e "${Fyellow}2.${plain} обновляет zator (код, листы, lua), ${Fyellow}3.${plain} — только Web-панель, ${Fyellow}4.${plain} ставит"
  echo -e "номерной релиз с закреплением версии, ${Fyellow}6.${plain} переустанавливает сам zapret2."
  echo -e "Пользовательские файлы (${green}netrogat.txt${plain}, ${green}TCP_Custom${plain}) и зафиксированные стратегии"
  echo -e "обновление не сбрасывает; к эталону их возвращает только явный сброс — ${Fyellow}п.8${plain} (выборочно)"
  echo -e "или ${Fyellow}п.7 -> п.3${plain} (полный). Перед сбросом меню предложит бэкап — соглашайся."
  echo -e "Красное ${red}«⬆ Есть новый конфиг от <дата>»${plain} в шапке меню: скачался новый config.default."
  echo -e "Примени его через ${Fyellow}п.5 -> п.7${plain} (п.1 — без скачивания): локи и client-scope сохранятся,"
  echo -e "zapret2 перезапустится сам. Нет доступа к GitHub — архив в /tmp и ${Fyellow}п.5 -> п.5${plain},"
  echo -e "зеркало (${Fyellow}п.5 -> п.10${plain}) или офлайн-сборка zator-offline-*.tar.gz (ставит и zator,"
  echo -e "и zapret2 полностью без сети)."
  echo ""
  ...
  echo -e "- Перед обновлениями и экспериментами делай бэкап (пункт 21): обновление пользовательские"
  echo -e "  файлы сохраняет, но копия рабочей схемы ускорит откат."
  echo -e "- Шапка меню (Город/провайдер, ${yellow}Платформа${plain}, ${yellow}Аптайм${plain}, ${yellow}RAM${plain}, версии config/zator/zapret2) —"
  echo -e "  экспресс-диагностика: обращаясь за помощью, копируй её целиком."
  echo ""
  pause_enter
}
